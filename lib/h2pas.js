...
class DefineList extends EventEmitter {
    #name = ''; #not = false;
    get name() { return this.#name; }
    constructor() { super(); }
}
class ProcessFile extends EventEmitter {
    #id = crypto.randomBytes(16).toString("hex"); #fileName = ''; #pathList = []; #fulldata = new Uint8Array();
    #DefineList = new Map();
    get id() { return this.#id; }
    /**Обработка файла Парсинг и преобразование
     * @param {String} fileName Путь до файла *.h
     * @param {String[]} pathList Пути поиска всех включений */
    constructor(fileName, pathList = ['./']) {
        super();
        this.#fileName = fileName; this.#pathList = pathList;
        if (!fs.existsSync(this.#fileName)){ throw `Файла не существует ${his.#fileName}`; }
        else { this.#fulldata = fs.readFileSync(this.#fileName).toString(); };
    }
    /** Построчный парсинг файла
     * @param {Uint8Array} srcFile */
    parse(srcFile) { }
}
